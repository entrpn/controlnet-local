...
class Model:
    ...
    def process_canny(self, input_image, prompt, a_prompt, n_prompt,
                      ...
                      eta, low_threshold=100, high_threshold=200):
        apply_canny = CannyDetector()
        self.load_weight('canny')
        print("treshold:", low_threshold)
    ...
    def process_hough(self, input_image, prompt, a_prompt, n_prompt,
                      ...
                      ddim_steps, scale, seed, eta, detect_resolution=512, value_threshold=0.1,
    ...
    @torch.inference_mode()
    def process_hed(self, input_image, prompt, a_prompt, n_prompt, num_samples, image_resolution, ddim_steps, scale,
                    seed, eta, detect_resolution=512):
    ...
    def process_scribble_interactive(self, input_image, prompt, a_prompt,
        ...
        img = resize_image(HWC3(input_image[:, :, 0]),
                           image_resolution)
        H, W, C = img.shape
    ...
    def process_fake_scribble(self, input_image, prompt, a_prompt, n_prompt,
                              num_samples, image_resolution,
                              ddim_steps, scale, seed, eta, detect_resolution=512):
        apply_hed = HEDdetector()
        self.load_weight('scribble')
    ...
    def process_depth(self, input_image, prompt, a_prompt, n_prompt,
                      num_samples, image_resolution,
                      ddim_steps, scale, seed, eta, detect_resolution=384):
    ...
    def process_normal(self, input_image, prompt, a_prompt, n_prompt,
                       num_samples, image_resolution,
                       ddim_steps, scale, seed, eta, detect_resolution=384, bg_threshold=0.4):
        apply_midas = MidasDetector()
